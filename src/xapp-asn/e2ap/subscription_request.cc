...
subscription_request::subscription_request(void){
  ...
  for (unsigned int i = 0; i < action_array_size; i++){
    ...
    assert(action_array[i].value.choice.RICaction_ToBeSetup_Item.ricActionDefinition  != 0);
  }
...
subscription_request::~subscription_request(void){
  ...
  for (unsigned int i = 0; i < action_array_size; i++){
    ...
    free(action_array[i].value.choice.RICaction_ToBeSetup_Item.ricActionDefinition );
  }
...
bool subscription_request::set_fields( InitiatingMessage_t * init_msg, subscription_helper &helper){
  ...
  *ranfunction_ie = helper.get_function_id();
  ...
  ricsubscription_ie->ricAction_ToBeSetup_List.list;
  for(unsigned int i = 0; i < ref_action_array->size(); i ++){
    ...
    action_array[i].value.choice.RICaction_ToBeSetup_Item.ricActionDefinition->buf = (uint8_t *) (*ref_action_array)[i].get_definition();
    action_array[i].value.choice.RICaction_ToBeSetup_Item.ricActionDefinition->size = (*ref_action_array)[i].get_definition_size();
