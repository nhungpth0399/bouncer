...
void Xapp::startup_subscribe_requests(void ){
   ...
   for(int i = 0; i<sz; i++)
   {
	...
	// Add code
	e2sm_handler smHandler;
	int typeOfSM = e2sm_rc;

	unsigned char buf_evedef[SM_BUFFER_SIZE];
	size_t buf_size_evedef = SM_BUFFER_SIZE;
	memset(buf_evedef, 0, buf_size_evedef * sizeof(buf_evedef[0]));
	
	unsigned char buf_actdef[SM_BUFFER_SIZE];
	size_t buf_size_actdef = SM_BUFFER_SIZE;
	memset(buf_actdef, 0, buf_size_actdef * sizeof(buf_actdef[0]));

	res = smHandler.handleServiceModel(typeOfSM, &buf_evedef[0], &buf_actdef[0], &buf_size_evedef, &buf_size_actdef);
	std::string event_def( buf_evedef, buf_evedef + sizeof(buf_evedef) / sizeof(buf_evedef[0]) );
	std::string act_def( buf_actdef, buf_actdef + sizeof(buf_actdef) / sizeof(buf_actdef[0]) );
	
	int request_id = 1;
	int function_id = e2sm_rc;
	din.set_request(request_id);
	din.set_function_id(function_id);
	din.set_event_def(event_def.c_str(), event_def.length());

	int action_id = 1;
	int action_type = RICactionType_insert;
	int next_action = RICsubsequentActionType_continue;
	int time_to_wait = RICtimeToWait_w10ms;
	din.add_action(action_id, 
				   action_type,
				   (void*)act_def.c_str(), 
				   act_def.length(), 
				   next_action, 
