	...
	switch(message->mtype){
		...
		case (RIC_INDICATION):	
			if( message->mtype == 12050 )
			{
				...
				// add
				int header_size = e2pdu->choice.initiatingMessage->value.choice.RICindication.protocolIEs.list.size;
				for(int i=0; i<header_size-1; i++)
				{
					if(e2pdu->choice.initiatingMessage->value.choice. \
						RICindication.protocolIEs.list.array[i]->id == ProtocolIE_ID_id_RICindicationHeader)
					{			
						e2sm_indication_rc e2sm_indication;
						unsigned char *buf = e2pdu->choice.initiatingMessage->value.choice.RICindication.protocolIEs.list.array[i]->value.choice.RICindicationHeader.buf;
						size_t size = e2pdu->choice.initiatingMessage->value.choice.RICindication.protocolIEs.list.array[i]->value.choice.RICindicationHeader.size;
						e2sm_indication.decode_indication_header(buf, &size);
					}
					if(e2pdu->choice.initiatingMessage->value.choice. \
						RICindication.protocolIEs.list.array[i]->id == ProtocolIE_ID_id_RICindicationMessage)
					{
						e2sm_indication_rc e2sm_indication;
						unsigned char *buf = e2pdu->choice.initiatingMessage->value.choice.RICindication.protocolIEs.list.array[i]->value.choice.RICindicationMessage.buf;
						size_t size = e2pdu->choice.initiatingMessage->value.choice.RICindication.protocolIEs.list.array[i]->value.choice.RICindicationMessage.size;
						e2sm_indication.decode_indication_message(buf, &size);
					}
				}
                asn_fprint(stdout, &asn_DEF_E2AP_PDU, e2pdu);
				...
				unsigned char buf_control[BUFFER_SIZE];
				size_t buf_control_size = BUFFER_SIZE;
				control_request controlReQ;
				controlReQ.start_control_request(&buf_control[0], &buf_control_size);
                
				// strncpy((char*)message->payload, buf_control, rmr_payload_size(message));           
				memcpy(message->payload, (void *)(&buf_control[0]), buf_control_size);
				*resend = true;
                ASN_STRUCT_FREE(asn_DEF_E2AP_PDU, e2pdu);
			} 
